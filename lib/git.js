...
async function getCommitLog(from, to = 'HEAD') {
  // git log --reverse' --format="%h | %s | %b | %P"
  const gitLogFormat = ["%H", "%s", "%b", "%P"].join(LOG_FIELD_SEPARATOR) + LOG_COMMIT_DELIMITER;
  const gitLog = await execa('git', ['log','--first-parent',`--format=${gitLogFormat}`, `${from ? `${from}..` : ''}${to}`])
    .then(result => result.stdout + "\n");
  return gitLog.split(LOG_COMMIT_DELIMITER + '\n').slice(0, -1)
    ...
    .map(commit => ({
      ...
      subject: commit[1],
      body: commit[2],
      parents: commit[3] ? commit[3].split(' ') : []
    }));
}
