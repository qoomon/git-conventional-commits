module.exports = function(config) {
   ...
   * @param {Object} generatorParams.firstChangelogCommit - the first commit to be considered for the changelog.
   * @param {Object} generatorParams.lastChangelogCommit - the last commit to be considered for the changelog.
  ...
  this.generateMarkdown = function (
    ...
    const includedCommits = commitLog
      .filter(
      ...
      .filter(
        ...
        (commit) =>
          ...
          (commit.type !== undefined && commitTypes.includes(commit.type)) // commit type in filter list
      )
      .filter(
    ...
    // mark the filtered commits as explicitly included
    includedCommits.forEach((commit) => (commit.includeInChangelog = true));

    // additionally include all commits that contain breaking changes
    const conventionalCommits = commitLog.filter(
      (commit) => commit.includeInChangelog || commit.breakingChanges.length > 0
    );
    ...
    if (conventionalCommits.length > 0) {
      ...
      const includedCommits = conventionalCommits.filter(
        (commit) => commit.includeInChangelog
      );
      ...
      const breakingCommits = Object.values(groupedBreakingChangelog)
