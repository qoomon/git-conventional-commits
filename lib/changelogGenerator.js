module.exports = function(config) {
  ...
  this.generateMarkdown = function(firstChangelogCommit, lastChangelogCommit, release, conventionalCommits, customMarkdown) {
    ...
    if (conventionalCommits.length > 0) {
      ...
      const includedCommits = conventionalCommits.filter(commit => commit.includeInChangelog);
      const groupedChangelog = bucketAggregation(includedCommits, commit => commit.type);
      ...
      const groupedBreakingChangelog = bucketAggregation(conventionalCommits, commit => commit.type);
