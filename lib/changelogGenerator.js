module.exports = function(config) {
  ...
  this.generateMarkdown = function(firstChangelogCommit, lastChangelogCommit, release, conventionalCommits, customMarkdown) {
    ...
    if (conventionalCommits.length > 0) {
      ...
      const commitTypes = Object.keys(groupedChangelog).map(e => e === 'undefined' ? undefined : e);
      commitTypes.sort((a, b) => - a.localeCompare(b)).reverse();
  ...
  function generateMarkdownCommitTypeSection(commitType, commits) {
    const typeSectionHeadline = config.headlines[commitType ? commitType : '?'] || commitType;
    let typeSectionMarkdown = `### ${typeSectionHeadline}\n`;

    const groupedCommits = bucketAggregation(commits, commit => commit.scope);
    const commitScopes = Object.keys(groupedCommits).map(e => e === 'undefined' ? undefined : e);
    commitScopes.sort((a, b) => - a.localeCompare(b)).reverse();
