...
exports.handler = async function (argv) {
    ...
    if (!conventionalCommit.type) {
        if (conventionalCommit.subject.startsWith("!squash ")
            || conventionalCommit.subject.endsWith("!fixup ")) {
            // Skip convention check for autosquash commits,
            // as they are not meant to be part of the final commit history
            return;
        }

        console.error('[ERROR] Invalid commit message');
        console.error(`  Valid message regex pattern: ${convention.msgRegex}`);
        process.exit(2);
    }
