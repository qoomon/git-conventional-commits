...
const hasMandatoryCaptureGroups = (regEx, captureGroupNames) => captureGroupNames.every(groupName => regEx.toString().includes(`<${groupName}>`));
...
function load(configFile) {
    ...
    if (configOverride.convention) {
        ...
        if (conventionOverride.msgMergeRegexList && conventionOverride.msgMergeRegexList.every(regEx => hasMandatoryCaptureGroups(regEx, ['description']))) {
            conventionConfig.msgMergeRegexList = conventionOverride.msgMergeRegexList;
        } else {
            console.warn(`[WARN] The merge RegEx list: ${conventionOverride.msgMergeRegexList.toString()} does not have the mandatory capture group description. The fallback is used`);
        }
