...
module.exports = function (convention, commitAnchor = 'HEAD', baseAnchor = undefined) {

    let commitLog

    this.getCommitLog = async function () {
        if (commitLog === undefined) {
            let lastReleaseTag = await getBaseTag();
    ...
    this.getVersion = async function () {
        ...
        const lastReleaseTag = await getBaseTag();
        if (lastReleaseTag) {
    ...
    // determines the changelog/version base, either the explicit `baseAnchor` (any ref e.g. tag,
    // branch or commit hash) or, by default, the last release tag reachable from `commitAnchor`,
    // resolved once and cached for subsequent calls
    async function getBaseTag() {
        if (baseAnchor === undefined) {
            baseAnchor = await getLastReleaseTag(commitAnchor);
        }
        return baseAnchor;
    }
