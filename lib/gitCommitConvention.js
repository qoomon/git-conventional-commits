...
module.exports = function (convention, commitAnchor = 'HEAD') {
    ...
    this.getCommitLog = async function () {
        if (commitLog === undefined) {
            ...
            } else if (await Git.isEqualCommit(await getLastTag(commitAnchor), commitAnchor)) {
                // commitAnchor itself is a pre-release tag, use it as the changelog boundary
                lastReleaseTag = await getLastTag(commitAnchor + '~1')
            }
    ...
    // finds the last tag reachable from commitRef, including pre-release versions
    async function getLastTag(commitRef) {
        return await Git.getLastTag({
            commitAnchor: commitRef,
            matchPattern: convention.releaseTagGlobPattern,
        })
