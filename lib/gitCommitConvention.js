...
module.exports = function(convention, commitRef = 'HEAD') {

  let commitLog

  this.getCommitLog  = async function() {
    if(commitLog === undefined){
      let lastReleaseTag = await getLastReleaseTag(commitRef)
      if(await Git.isEqualCommit(lastReleaseTag, commitRef)) {
        lastReleaseTag = await getLastReleaseTag(commitRef + '~1')
      }
      const commits = await Git.getCommitLog(lastReleaseTag, commitRef);
  ...
  this.getVersion = async function() {
    ...
    const lastReleaseTag = await getLastReleaseTag(commitRef);
    if (lastReleaseTag) {
    ...
    if(! await Git.isEqualCommit(lastReleaseTag, commitRef)) {
      ...
      const changes = (await this.getCommitLog()).reduce((acc, commit) => {
          ...
          return acc;
      }, {breaking: 0, feature: 0, patch: 0});
      // TODO rename to breakingChanges, features and patches
