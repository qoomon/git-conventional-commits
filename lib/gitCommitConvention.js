...
module.exports = function (convention, commitAnchor = 'HEAD', baseAnchor = undefined) {
    ...
    let commitLog
    let baseAnchorPromise

    async function resolveBaseAnchor() {
        if (baseAnchorPromise === undefined) {
            baseAnchorPromise = baseAnchor === undefined ? getLastReleaseTag(commitAnchor) : Promise.resolve(baseAnchor);
        }
        return baseAnchorPromise;
    }

    this.getCommitLog = async function () {
        if (commitLog === undefined) {
            let lastReleaseTag = await resolveBaseAnchor();
    ...
    this.getVersion = async function () {
        ...
        const lastReleaseTag = await resolveBaseAnchor();
        if (lastReleaseTag) {
